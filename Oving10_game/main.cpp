  ...
  void start() {
    ...
    while (running) {
      ...
      if (ImGui::SliderFloat("Horizontal ball position", &horizontal_position, 0.0, 10.0)) {
        btTransform transform;
        transform.setIdentity();
        transform.setOrigin(btVector3(horizontal_position * 0.2, world.falling_sphere.body.getCenterOfMassPosition().getY(), 0.0));
      ...
      if (ImGui::VSliderFloat("Vertical ball position", {20, 100}, &vertical_position, 0.0, 10.0)) {
        btTransform transform;
        transform.setIdentity();
        transform.setOrigin(btVector3(world.falling_sphere.body.getCenterOfMassPosition().getX(), vertical_position * 0.5, 0.0));
        world.falling_sphere.body.setCenterOfMassTransform(transform);
      }
      ImGui::End();
